...
#### Population-weighted decomposition: Income
```{r}
...
rp_wtdecomp <- rp_wtdecomp %>% rename(wtotal=gini_total, 
                                      wwithin=gini_within, wbetween=gini_between, woverlap=gini_overlap)
#rp_wtdecomp_p <- rp_wtdecomp %>% mutate(within_p = within/total, between_p = between/total, overlap_p = overlap/total)
```

</br>

#### Data preparation: decomposition by INCOME Class
```{r}
...
#### Plot decompositions by INCOME class
```{r fig.height = 3, fig.width = 3, fig.align = "center"}
...
#### Decomposition by LCV, Energy Expenditure and Income
