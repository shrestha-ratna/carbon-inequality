...
## output data
dat_21_sel <- dat_21 %>% select(State, pc_CO2, Party, HDD, CDD, in_class, HL_Expend,  HL_price)
file.output <- file.path(dir.outdata, "dat_21_selected.csv")
write.table(dat_21_sel, file.output, sep=",", row.names=FALSE, col.names=TRUE, quote=FALSE)
```
...
wc_97_ex <- wc_97 %>% select(State.Abb, State, pop_mi, pc_CO2, cum_unit, cum_co2)
file.output <- file.path(dir.outdata, "cumulative_co2_1997.csv")
write.table(wc_97_ex, file.output, sep=",", row.names=FALSE, col.names=TRUE, quote=FALSE)
...
colors <- c("Overall (0.244, 1.11)" = "green", "Coal (0.462, 0.79)"= "#080808", "Petroleum (0.162, 1.32)" = "brown", 
             "Natural Gas (0.327, 1.18)" = "#00A9FF")
...
wc_19_ex <- wc_19 %>% select(State.Abb, State, pop_mi, pc_CO2, cum_unit, cum_co2)
file.output <- file.path(dir.outdata, "cumulative_co2_2019.csv")
write.table(wc_19_ex, file.output, sep=",", row.names=FALSE, col.names=TRUE, quote=FALSE)
...
#### Lorenz curves 2019
```{r fig.height = 3, fig.width = 5, fig.align = "center"}
colors <- c("Overall (0.238, 0.92)" = "green", "Coal (0.570, 0.90)"= "#080808", "Petroleum (0.149, 1.32)" = "brown", 
             "Natural Gas (0.265, 1.12)" = "#00A9FF")
...
#### Lorenz curves: 2020
```{r fig.height = 3, fig.width = 5, fig.align = "center"}
colors <- c("Overall (0.255, 1.11)" = "green", "Coal (0.595, 0.76)"= "#080808", "Petroleum (0.175, 1.22)" = "brown", 
             "Natural Gas (0.271, 1.08)" = "#00A9FF")
...
#### Lorenz curves 2021
```{r fig.height = 3, fig.width = 5, fig.align = "center"}
colors <- c("Overall (0.250, 1.11)" = "green", "Coal (0.595, 0.76)"= "#080808", "Petroleum (0.175, 1.22)" = "brown", 
             "Natural Gas (0.271, 1.08)" = "#00A9FF")
...
#### Lorenz curve 1997
```{r fig.height = 3, fig.width = 5, fig.align = "center"}
colors <- c("Industrial (0.447, 1.16)" = "#4682B4", "Electricity (0.379, 0.83)" = "darkorange", "Residential (0.294, 0.80)" = "purple", "Commercial (0.258, 0.84)" = "#43464B", "Transport (0.135, 1.12)" = "yellow")
...
colors <- c("Industrial (0.511, 0.99)" = "#4682B4", "Electricity (0.394, 0.89)" = "darkorange", "Residential (0.339, 0.86)" ="purple", "Commercial (0.238, 0.88)" = "#43464B", "Transport (0.108, 1.15)" = "yellow")
...
#### Lorenz curves 2020
```{r fig.height = 3, fig.width = 5, fig.align = "center"}
colors <- c("Industrial (0.511, 1.02)" = "#4682B4", "Electricity (0.392, 0.87)" = "darkorange", "Residential (0.329, 0.88)" = "purple", "Commercial (0.230, 0.87)" = "#43464B", "Transport (0.138, 1.17)" = "yellow")
...
#### Lorenz curves 2021
```{r fig.height = 3, fig.width = 5, fig.align = "center"}
colors <- c("Industrial (0.517, 1.01)" = "#2E6B8A", "Electricity (0.395, 0.86)" = "darkorange", "Residential (0.326, 0.88)" = "purple", "Commercial (0.228, 0.87)" = "#43464B", "Transport (0.132, 1.24)" = "yellow")
...
sector_fuel_lorenz <- annotate_figure(sector_fuel_lorenz, left =text_grob("Cumulative per capita CO2", vjust=3, 
                      rot =90, size =8), bottom = text_grob("Cumulative population (ranked by respective per capita CO2)", 
                      vjust=0, size =8))
```
...
#### Hypothetical Lorenz curves with the same Gini but different LAC
```{r tidy=TRUE}
file.lorenz_lac <- file.path(dir.data, "lorenz_lac.csv")
dat_ll <- read.csv(file.lorenz_lac, header=TRUE, stringsAsFactors=FALSE)
```

#### Gini and LAC
```{r tidy=TRUE}
gini_y12 <- dat_ll %>% summarise(giniy1 = gini(y1), giniy2 = gini(y2))

lac_12 <- dat_ll %>% summarise(lac1 = Lasym(y1), lac2 = Lasym(y2))
```

#### method:Data preparation
```{r}
m1 <- dat_ll %>% arrange(y1)
m1$one <- "1" 
m1$one <-as.numeric(m1$one)
m1$cum_u1 = cumsum(m1$one/sum(m1$one))
m1$cum_y1 = cumsum(m1$y1/sum(m1$y1))

m2 <- dat_ll %>% arrange(y2)
m2$one <- "1" 
m2$one <-as.numeric(m2$one)
m2$cum_u2 = cumsum(m2$one/sum(m2$one))
m2$cum_y2 = cumsum(m2$y2/sum(m2$y2))
```

#### Gini and LAC
```{r tidy=TRUE}
colors <- c("L1 (0.4, 1.07)" = "#ABA300", "L2 (0.4, 0.80)"= "#00A9FF")

LL_plot <- ggplot() + 
  geom_abline(linetype = "dashed", color = "grey")+
  geom_line(aes(x=m1$cum_u1, y=m1$cum_y1, color = "L1 (0.4, 1.07)"))+
  geom_line(aes(x=m2$cum_u2, y=m2$cum_y2, color = "L2 (0.4, 0.80)"))+
  labs(x = "Cumulative share of population", y = "Cumulative share of CO2")+
  ...
  #theme(plot.margin = unit(c(5.5, 1.5, 5.5, 1.5), "pt"))+
  scale_color_manual(values = colors, breaks = names(colors)[c(1, 2)])
```

#### Lorenz plot all together (overall CO2, Sector and Fuel Type)
```{r fig.height = 2.5, fig.width =3.25, fig.align = "center"}

file.plot <- file.path(dir.plot, "LL_plot.png")
png(file.plot, height= 2.5, width=3.25, units="in", bg="white", res=300)
LL_plot
dev.off()
```
