...
all_decomp <- annotate_figure(sector_fuel_decomp, left = text_grob("Decomposition of per capita CO2 Gini", rot=90, size=9))
...
#### Shapley decomposition of overall CO2 Gini by Year and Party  (Dem, Rep and Neutral)
##### Data preparation: unweighted 
```{r}
## Data preparation
foo3 <- function(dat_p){
  ...
  keys_ha <- keys(ha)
  for(i in keys_ha) {
    bar <- gini_decomp(x=ha[[i]]$pc_CO2, z=ha[[i]]$Party_3)
    hb[[i]] <- bar["gini_decomp"]
  }
  return(hb)
}
ung_decomp3 <- foo3(dat_p)

### PARSE HASH FUNCTION OUTPUT
list_decomp3 <- as.list(ung_decomp3)
yt3 <- lapply(list_decomp3, function(x) x[1]$gini_decomp)
df_decomp3 <- do.call(rbind.data.frame, yt3)
df_decomp3 <- cbind(Year=as.numeric(rownames(df_decomp3)), df_decomp3)
rownames(df_decomp3) <- NULL
df_decomp3 <- df_decomp3[order(df_decomp3$Year, decreasing = FALSE),]
g_decomp3 <- df_decomp3 %>% rename(total=gini_total, 
                                within=gini_within, between=gini_between, overlap=gini_overlap)
```

#### Data Preparation: Population-weighted
```{r}
wtfoo3 <- function(dat_p){
  ha <-hash()
  for(j in 1997:2021){
    ha[[as.character(j)]] <-by_year(dat_p, j)
  } 
  ...
  keys_ha <- keys(ha)
  for(i in keys_ha) {
    bar <- gini_decomp(x=ha[[i]]$pc_CO2, z=ha[[i]]$Party_3, weights = ha[[i]]$pop_mi)
    hb[[i]] <- bar["gini_decomp"]
  }
  return(hb)
}
wtg_decomp3 <- wtfoo3(dat_p)

### PARSE HASH FUNCTION OUTPUT
list_wtdecomp3 <- as.list(wtg_decomp3)
wtyt3 <- lapply(list_wtdecomp3, function(x) x[1]$gini_decomp)
df_wtdecomp3 <- do.call(rbind.data.frame, wtyt3)
df_wtdecomp3 <- cbind(Year=as.numeric(rownames(df_wtdecomp3)), df_wtdecomp3)
rownames(df_wtdecomp3) <- NULL
df_wtdecomp3 <- df_wtdecomp3[order(df_wtdecomp$Year, decreasing = FALSE),]
wtg_decomp3 <- df_wtdecomp3 %>% rename(wtotal=gini_total, 
                                     wwithin=gini_within, wbetween=gini_between, woverlap=gini_overlap)
```
</br>
...
#### Merge weighted and unweighted prepared data for decomposition
```{r}
decomp_party_3 <-left_join(wtg_decomp3, g_decomp3)
decomp_party_3 <- decomp_party_3[order(decomp_party_3$Year),]

### WRITE OUTPUT FILE --
file.output <- file.path(dir.outdata, "decomposition_party_3.csv")
write.table(decomp_party_3, file.output, sep=",", row.names = FALSE, col.names = TRUE, quote=FALSE)
```

#### Data preparation for a plot
```{r fig.height = 5, fig.width = 7, fig.align = "center"}
dparty3 <- reshape2::melt(decomp_party_3, id.vars="Year")
dparty3$Group <- ifelse((dparty3$variable == "wtotal") | (dparty3$variable == "total"), "Overall",
                     ifelse((dparty3$variable == "wwithin") | (dparty3$variable == "within"), "Within group",
                            ifelse((dparty3$variable == "wbetween") | (dparty3$variable == "between"),"Between group","Overlap")))


dparty3$Weight <- ifelse((dparty3$variable == "wtotal") | (dparty3$variable == "wwithin") | 
                          (dparty3$variable == "wbetween") | (dparty3$variable == "woverlap"), "Population weighted", "Unweighted")

dparty3$Group <- factor(dparty3$Group, levels=c("Overall","Within group", "Between group", "Overlap"))
dparty3$Weight <- factor(dparty3$Weight, levels=c("Population weighted","Unweighted"))

#table(dparty$variable, dparty$Group)
#table(dparty$variable, dparty$Weight)
```

#### Plot: Decomposition of overall per capita CO2 Gini by 3-PARTY and YEAR
```{r fig.height = 2, fig.width = 4, fig.align = "center"}
pparty_3 <-ggplot(dparty3, aes(x=Year))+
  geom_line(aes(y = value, color = Group, linetype = Weight), size=0.7) +
  scale_colour_manual(values=c("black", "#4DAF4A","#8494FF", "#ED68ED"), theme(legend.justification = "left")) +  
  scale_linetype_manual(values=c("solid","dotted")) +
  labs(x = "Year", y = "Decomposition of CO2 Gini")+
  theme(text=element_text(size=8))+
  scale_x_continuous(breaks=c(1997, 2000, 2003, 2006, 2009, 2012, 2015, 2019, 2021)) +
  #scale_x_continuous(breaks = seq(min(dparty$Year), max(dparty$Year), 1)) +
  scale_y_continuous(limits = c(0, 0.36), breaks = seq(0, 0.36, 0.04)) +
  ...
  theme(legend.background = element_rect(fill = "transparent"))+
  theme(legend.position = "bottom", legend.margin=margin(t=-2)) +
  theme(legend.box = "vertical", legend.spacing.x = unit(0.2, 'cm'))+
  theme(legend.key.height = unit(0.3, 'cm'))+
  theme(legend.key.width = unit(0.3, 'cm'))+
  theme(legend.text = element_text(size=7))
```

#### Shapley decomposition of per capita CO2 MLD by Year and Party  (Dem, Rep and Neutral)
##### Data preparation: unweighted 
```{r}
## Data preparation
foomld <- function(dat_p){
  ha <-hash()
  for(j in 1997:2021){
    ha[[as.character(j)]] <-by_year(dat_p, j)
  } 
  ...
  keys_ha <- keys(ha)
  for(i in keys_ha) {
    bar <- mld_decomp(x=ha[[i]]$pc_CO2, z=ha[[i]]$Party_3)
    hb[[i]] <- bar["mld_decomp"]
  }
  return(hb)
}
ung_decompmld <- foomld(dat_p)

### PARSE HASH FUNCTION OUTPUT
list_decompmld <- as.list(ung_decompmld)
ytmld <- lapply(list_decompmld, function(x) x[1]$mld_decomp)
df_decompmld<- do.call(rbind.data.frame, ytmld)
df_decompmld <- cbind(Year=as.numeric(rownames(df_decompmld)), df_decompmld)
rownames(df_decompmld) <- NULL
df_decompmld <- df_decompmld[order(df_decompmld$Year, decreasing = FALSE),]
g_decompmld <- df_decompmld %>% rename(total=mld_total, 
                                within=mld_within, between=mld_between)
```

#### Data Preparation: Population-weighted
```{r}
wtfoomld <- function(dat_p){
  ha <-hash()
  for(j in 1997:2021){
    ha[[as.character(j)]] <-by_year(dat_p, j)
  } 
  ...
  keys_ha <- keys(ha)
  for(i in keys_ha) {
    bar <- mld_decomp(x=ha[[i]]$pc_CO2, z=ha[[i]]$Party_3, weights = ha[[i]]$pop_mi)
    hb[[i]] <- bar["mld_decomp"]
  }
  return(hb)
}
wtg_decompmld <- wtfoomld(dat_p)

### PARSE HASH FUNCTION OUTPUT
list_wtdecompmld <- as.list(wtg_decompmld)
wtytmld <- lapply(list_wtdecompmld, function(x) x[1]$mld_decomp)
df_wtdecompmld <- do.call(rbind.data.frame, wtytmld)
df_wtdecompmld <- cbind(Year=as.numeric(rownames(df_wtdecompmld)), df_wtdecompmld)
rownames(df_wtdecomp3) <- NULL
df_wtdecompmld <- df_wtdecompmld[order(df_wtdecompmld$Year, decreasing = FALSE),]
wtg_decompmld <- df_wtdecompmld %>% rename(wtotal=mld_total, 
                                     wwithin=mld_within, wbetween=mld_between)
```
</br>

#### Merge weighted and unweighted prepared data for decomposition
```{r}
decomp_party_3mld <-left_join(wtg_decompmld, g_decompmld)
decomp_party_3mld <- decomp_party_3mld[order(decomp_party_3mld$Year),]

### WRITE OUTPUT FILE --
file.output <- file.path(dir.outdata, "decomposition_party_3mld.csv")
write.table(decomp_party_3mld, file.output, sep=",", row.names = FALSE, col.names = TRUE, quote=FALSE)
```

#### Data preparation for a plot
```{r fig.height = 5, fig.width = 7, fig.align = "center"}
dparty3mld <- reshape2::melt(decomp_party_3mld, id.vars="Year")
dparty3mld$Group <- ifelse((dparty3mld$variable == "wtotal") | (dparty3mld$variable == "total"), "Overall",
                     ifelse((dparty3mld$variable == "wwithin") | (dparty3mld$variable == "within"), "Within group", 
                            "Between group"))


dparty3mld$Weight <- ifelse((dparty3mld$variable == "wtotal") | (dparty3mld$variable == "wwithin") | 
                          (dparty3mld$variable == "wbetween"), "Population weighted", "Unweighted")

dparty3mld$Group <- factor(dparty3mld$Group, levels=c("Overall","Within group", "Between group"))
dparty3mld$Weight <- factor(dparty3mld$Weight, levels=c("Population weighted","Unweighted"))

#table(dparty$variable, dparty$Group)
#table(dparty$variable, dparty$Weight)
```

#### Plot: Decomposition of overall per capita CO2 MLD by 3-PARTY and YEAR
```{r fig.height = 2, fig.width = 4, fig.align = "center"}
pparty_3mld <-ggplot(dparty3mld, aes(x=Year))+
  geom_line(aes(y = value, color = Group, linetype = Weight), size=0.7) +
  scale_colour_manual(values=c("black", "#4DAF4A","#8494FF", "#ED68ED"),theme(legend.justification = "right")) +  
  scale_linetype_manual(values=c("solid","dotted")) +
  labs(x = "Year", y = "Decomposition of CO2 MLD")+
  theme(text=element_text(size=8))+
  scale_x_continuous(breaks=c(1997, 2000, 2003, 2006, 2009, 2012, 2015, 2019, 2021)) +
  #scale_x_continuous(breaks = seq(min(dparty$Year), max(dparty$Year), 1)) +
...
### decomposition with 3 party types
```{r fig.height = 2.75, fig.width = 6.75, fig.align = "center"}
decomp3_mld_Gini <- cowplot::plot_grid(pparty_3, pparty_3mld, labels = c("a", "b"), label_size = 8,
                              label_fontface = "bold", nrow = 1, ncol = 2)

file.plot <- file.path(dir.plot, "decomp3_mld_Gini.png")
png(file.plot, height =2.75, width = 6.75, units="in", bg= "white", res=300)
decomp3_mld_Gini
dev.off()
```
